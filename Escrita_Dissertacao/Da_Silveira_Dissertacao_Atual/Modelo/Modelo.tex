...
%TODO Rever citação de Dutt e Nikiforos

%TODO Checar consistência entre variáveis no código e no texto
...
\end{equation}
%TODO Rever se vai mantar em tempo contínuo
...
INCLUIR DISCUSSÃO MILGATE E DIFERENTES FECHAMENTOS? (2 LINHAS)
...
\begin{equation}
\label{_h}
    \dot h = h_{-1}\cdot \gamma_u\cdot (u - \overline{u}_N)
\end{equation}
...
\begin{equation}
    r_m = \overline r_m
\end{equation}
como hipótese simplificadora, os referidos $\sigma_s$ são nulos de modo que tanto empréstimo quanto hipotecas sejam remunerados à taxa dos depósitos. Nesses termos, o saldo financeiro líquido dos bancos ($NFW_b$) é definido como o pagamento de juros recebidos descontadas as remunerações dos depósitos:
\begin{equation}
